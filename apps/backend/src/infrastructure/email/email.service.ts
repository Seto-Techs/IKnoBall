...
export class EmailService {
  ...
  private getEmailQueueConfig() {
    ...
    const fromAddressMap = this.config.get<EmailFromAddressMap | undefined>(
  ...
  async sendVerifyEmail(params: {
    ...
    const props: EmailTemplateProps = {
      ...
      bodyText:
        params.bodyText ??
        `We received a request to verify your email address. Click the button below to confirm your account and get started.`,
      buttonText: params.buttonText ?? 'Verify Account',
      buttonUrl: params.link,
      footerText:
        params.footerText ?? `If you didn't request this, you can safely ignore this email.`,
      iconType: 'verify',
    };
