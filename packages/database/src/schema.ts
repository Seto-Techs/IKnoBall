...
export const users = pgTable('users', {
  id: id(), email: text('email').notNull().unique(), name: text('name').notNull().default('User'),
  passwordHash: text('password_hash').notNull(), isVerified: boolean('is_verified').notNull().default(false),
  verifiedAt: timestamp('verified_at', { precision: 3 }), isActive: boolean('is_active').notNull().default(true),
  resetToken: text('reset_token'), resetTokenExpiry: timestamp('reset_token_expiry', { precision: 3 }),
...
export const userSessions = pgTable('user_sessions', {
  ...
  userAgent: text('user_agent'), ipAddress: text('ip_address'), refreshTokenHash: text('refresh_token_hash'),
  lastUsedAt: timestamp('last_used_at', { precision: 3 }).notNull().defaultNow(),
  expiresAt: timestamp('expires_at', { precision: 3 }).notNull(), revokedAt: timestamp('revoked_at', { precision: 3 }),
  createdAt: timestamp('created_at', { precision: 3 }).notNull().defaultNow(),
